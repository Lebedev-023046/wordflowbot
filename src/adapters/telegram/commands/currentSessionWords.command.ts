...
import type { GetCurrentSessionWordsUseCase } from '../../../application/session/queries/GetCurrentSessionWordsUseCase';
...
} from '../lib/currentSessionWordsPagination';

export function registerCurrentSessionWordsCommand(
  bot: Telegraf,
  getCurrentSessionWordsUseCase: GetCurrentSessionWordsUseCase,
) {
  const handleShowWords = async (ctx: Context) => {
    const userId = getUserId(ctx);
    const result = await getCurrentSessionWordsUseCase.execute(userId);
