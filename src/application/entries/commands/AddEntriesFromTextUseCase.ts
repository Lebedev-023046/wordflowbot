...
interface AddEntriesFromTextCommand {
  sessionId: string;
  text: string;
}

export type AddEntriesFromTextResult =
  | { kind: 'empty' }
  | { kind: 'duplicatesOnly' }
  | { count: number; entries: Entry[]; kind: 'saved' };

export class AddEntriesFromTextUseCase {
  ...
  async execute({
    sessionId,
    text,
  }: AddEntriesFromTextCommand): Promise<AddEntriesFromTextResult> {
