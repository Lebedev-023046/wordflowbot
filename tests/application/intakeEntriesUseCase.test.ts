...
import { IntakeEntriesUseCase } from '../../src/application/use-cases/IntakeEntriesUseCase';
import type { EntryRepository } from '../../src/entities/entry/api/entryRepository';

test('IntakeEntriesUseCase saves non-conflicting entries when saveMany hits a duplicate race', async () => {
  ...
  const result = await new IntakeEntriesUseCase(
    entries,
    parser,
    entryFactory,
  ).execute({
...
test('IntakeEntriesUseCase returns duplicatesOnly when every entry loses the duplicate race', async () => {
  const entryFactory = new EntryFactory();
  const parser = new EntryParser();
