...
test('GetSessionHistoryUseCase returns finished sessions with default titles and completed word counts', async () => {
  ...
  const result = await new GetSessionHistoryUseCase(sessions, entries).execute(
    1,
  );
...
test('GetSessionHistoryUseCase returns empty when there are no finished sessions', async () => {
  const sessions = new InMemorySessionRepository();
  const entries = new InMemoryEntryRepository();
